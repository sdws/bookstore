...
class BooksManager(models.Manager):
	...
	def get_books_by_type(self,type_id,limit=None,sort='default'):
		...
		if sort == 'new':
			...
			order_by = ('-pk',) #安装primary key(主键id)降序排列

		#查询数据
		books_li = self.filter(type_id=type_id).order_by(*order_by)
		#查询结果集的限制,取出一截列表,下标为0到limit-1元素的
		if limit:
			books_li = books_li[:limit]
		return books_li

	def get_bools_by_id(self,books_id):
		'''根据商品的id获取商品信息'''
		try:
...
class Books(BaseModel):
	...
	#Field 字段.域  Decimal 小数,十进制  digits位数  decimal_places小数的位数
