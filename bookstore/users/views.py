...
def user(request):
	...
	for id in history_li:
		books = Books.objects.get_bools_by_id(books_id=id)
...
def verifycode(request):
	...
	#构造字体对象
	#os.path.join(settings.BASE_DIR, 'Library/Fonts/Arial.ttf'  表示跟目录衔接子目录,一直到其中的文件
	font = ImageFont.truetype(os.path.join(settings.BASE_DIR, 'Library/Fonts/Arial.ttf'),15)
	#构造文字颜色
